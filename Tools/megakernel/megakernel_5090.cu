...
#error "megakernel_5090.cu requires sm_75 or later. Use -arch=sm_75 for Tesla T4."
#endif
        ...
        if (threadIdx.x == 0) {
            ...
            if (arrived == nblocks - 1) {
                *counter = 0;
                ...
                while (*vgen <= my_gen) {
                    for (int i = 0; i < 32; i++) { asm volatile("" ::: "memory"); }
                }
            }
            local_gen = my_gen + 1;
        }
...
static inline void ldg_configure_kernel_attributes() {
    ...
    cudaFuncSetAttribute(ldg_decode_kernel_persistent,
                         ...
                         64 * 1024);    // 64 KB — T4 max shared mem
